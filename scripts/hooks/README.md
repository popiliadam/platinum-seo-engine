...
| `ai_disclosure_rescan.py` | `hooks/post-tool-use.json` | `PostToolUse` — AMO batch-2e AI-disclosure surface rescan (the POST-write quarantine twin of `validate_content_write.py`). Added as the **2nd** `PostToolUse` command (`audit_post_tool_use.py` untouched; the batch-0a `env_probe.py` that previously sat between them was UNWIRED in codex-hostile-audit #17; matcher `Edit\|Write\|Bash`). A PostToolUse hook fires AFTER the write, so it cannot prevent — it must **detect + revert**: after any Edit/Write/Bash it re-scans a *just-written* blog-HTML file (`outputs/{blog,content}/…/*.html`; Write/Edit `file_path` or a Bash `.html` token that EXISTS and has a fresh mtime — a mere `cat <file>` READ leaves mtime old, so it is never quarantined) via `content_validator.validate_content`. On a RED `AI-disclosure` finding it **quarantine-renames** the file off the live surface (`os.replace` → `<path>.BLOCKED-ai-disclosure`; the live `.html` is gone, the content is preserved for operator review) and emits `{"decision":"block","reason":…}` to stdout telling the model to rewrite without the disclosure. This closes the **Bash/heredoc bypass** of the PreToolUse Write-gate (the demo-furniture `vcc-` pattern writes the body with a `cat > … << 'EOF'` heredoc, which never invokes Write/Edit). REUSES `content_validator` + `validate_content_write.is_content_html_path` + `_resolve_profile` (no duplicated detection); a Bash path built from a shell variable is a documented Path-A limit. Non-blocking-on-error (a rescan bug → exit 0). Calls `scripts/validation/content_validator` + `scripts/hooks/validate_content_write` |
| `check_events_writer.py` | `hooks/pre-tool-use.json` | `PreToolUse` — events.jsonl writer policy (matcher `Edit\|Write\|NotebookEdit\|Bash`), added as the **last** command of the first block. Blocks (`exit 2`) a DIRECT write to a live `projects/*/_state/events.jsonl`: a `>`/`>>` redirect, `tee`, `sed -i`, a `cp`/`mv` whose **destination** is the ledger, or a `Write`/`Edit` payload — including when the write is not the leading token (`cd x && echo … >> ledger`). The deny names the remedy: `scripts/state/events_writer.py`, which validates against `events.schema.json` BEFORE appending and raises `EventValidationError` rather than persisting a bad row. Deliberately ASYMMETRIC — **reading** the ledger is always allowed (`cat`/`grep`/`wc -l <`/`jq`, and a `cp` whose *source* is the ledger), because diagnosis, monthly reporting and `migrate_legacy_events.py` all read it constantly. `events.jsonl.legacy` / `.bak` are the migration's own outputs and are NOT the live ledger. Closes the boundary the 2026-07-09..08-06 drift walked through: 93 hand-written rows across six projects, invisible to the pre-commit guards (the ledger is gitignored) and un-flaggable by `check_append_only.sh` (an append IS append-only). Escape hatch for intentional migration/recovery: `PSEO_EVENTS_WRITER=events_writer.py`. Pure + READ-ONLY; fail-OPEN on an unreadable payload |
