...
|---|---|---|
| `1.3` (7) | adstark-tr, bigcat-tr, dentnotion, eykom, lastiksa-tr, noran-insaat-tr, vento | 0004 then 0005 |
| `1.4` (1) | iwallet-tr | 0005 only |
| `1.5` (2) | aluminumstation-ca, miningaa-com | SKIP (already current) |
...
- [ ] **WS.1 — Pre-flight.** In the workspace repo: capture the starting point and branch.
  ...
  ```
  Confirm exactly 7×1.3, 1×1.4 (iwallet-tr), 2×1.5 (aluminumstation-ca, miningaa-com).
...
- [ ] **WS.2 — Dry-run first (no writes).** For one 1.3 project, preview both steps to confirm the
  ...
  python3 $ENG/scripts/migrations/migration_0004_project_config_1_3_to_1_4.py projects/dentnotion/project.config.json --dry-run
  ```

- [ ] **WS.3 — Migrate the 7 × 1.3 projects (in-place, chain 0004→0005).**
  ```bash
  ENG=/Users/apple/Documents/platinum-seo-engine
  for p in adstark-tr bigcat-tr dentnotion eykom lastiksa-tr noran-insaat-tr vento; do
...
- [ ] **WS.4 — Migrate iwallet-tr (1.4 → 1.5, 0005 only).**
  ```bash
  python3 $ENG/scripts/migrations/migration_0005_project_config_1_4_to_1_5.py projects/iwallet-tr/project.config.json
  ```
...
- [ ] **WS.7 — Commit in the WORKSPACE repo (the 8 configs only; NOT .bak, NOT runtime artifacts).**
  ```bash
  git add projects/adstark-tr/project.config.json projects/bigcat-tr/project.config.json \
          projects/dentnotion/project.config.json projects/eykom/project.config.json \
          projects/lastiksa-tr/project.config.json projects/noran-insaat-tr/project.config.json \
          projects/vento/project.config.json projects/iwallet-tr/project.config.json
...
- Read-only cross-check from the engine repo (proves the engine now sees a consistent workspace):
  ...
  PSEO_WORKSPACE_ROOT=/Users/apple/Documents/platinum-seo-workspace python3 scripts/state/dump_workspace.py --project dentnotion --json | head
  ```
  (Should succeed and show dentnotion. NOTE: full `--json` without `--project` only works AFTER
...
- The 2 already-1.5 projects (aluminumstation-ca, miningaa-com) — do not touch.
...
- Migrated (8): adstark-tr,bigcat-tr,dentnotion,eykom,lastiksa-tr,noran-insaat-tr,vento (1.3→1.5); iwallet-tr (1.4→1.5)
- Skipped (already 1.5): aluminumstation-ca, miningaa-com
