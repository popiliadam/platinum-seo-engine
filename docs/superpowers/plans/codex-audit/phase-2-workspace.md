...
- `scripts/state/dump_workspace.py` `_resolve_slug` (~lines 45–65): `slug = data.get("slug")` then
  ...
  `{"active_project": "demo-dental", "updated_at": "..."}`. Cross-check: `commands/pseo-active.md`
...
PSEO_WORKSPACE_ROOT=/Users/apple/Documents/platinum-seo-workspace python3 scripts/state/dump_workspace.py --json | head    # now SUCCEEDS (demo-dental), no "no 'slug' key"
