...
**Affects:** Aluminum Station CA, Noran İnşaat, Eykom (local-service hybrid).
...
**Affects:** iWallet TR (YMYL-high), Aluminum Station CA (local-service), Eykom (e-commerce + local-service hybrid).
...
### Task 6.1: iWallet TR pilot session

**Inputs needed from operator:**
- iWallet's LinkedIn company URL
- iWallet executives' LinkedIn profile URLs (for question 3 — sektörel deneyim)
- Approval on each discovered draft entry

- [ ] **Step 1: Run discovery stage**

```bash
/pseo-active iwallet-tr
# Then invoke brand-onboarding with full pipeline
```
The skill runs Stage A (DFS + Scrapling against iwallet.com.tr), returns draft entries.
...
For YMYL profile (iwallet-tr), minimum 3 approved experience entries required to proceed.
...
Skill writes approved entries to `projects/iwallet-tr/project.config.json`. Verify:
```bash
cat projects/iwallet-tr/project.config.json | jq '.content_settings.experience_database | length'
```
Expected: ≥3.
...
git add projects/iwallet-tr/project.config.json
git commit -m "feat(iwallet-tr): bank seed (≥3 experience entries, YMYL gate satisfied)"
```

---

### Task 6.2: Aluminum Station CA pilot session

Same protocol as Task 6.1 but for `aluminumstation-ca`. Profile is local-service only, so minimum is ≥1 experience entry.

- [ ] **Step 1: `/pseo-active aluminumstation-ca`**
...
```bash
git commit -m "feat(aluminumstation-ca): bank seed (≥1 experience entry, local-service gate satisfied)"
```

---

### Task 6.3: Eykom pilot session

Same protocol; Eykom profiles are `[e-commerce, local-service]`, so ≥1 experience entry minimum.

- [ ] **Step 1: `/pseo-active eykom`**
...
```bash
git commit -m "feat(eykom): bank seed (≥1 experience entry, hybrid profile satisfied)"
```
