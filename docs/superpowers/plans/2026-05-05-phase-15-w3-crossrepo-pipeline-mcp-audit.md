...
Dentnotion project: `/Users/apple/Documents/platinum-seo-workspace/projects/dentnotion`
...
```bash
grep -rn "platinum-seo-workspace\|/Documents/.*dentnotion\|/Users/.*workspace" \
...
8-slug consistency check (engine knows only abstract slug, not dentnotion literal):
```bash
# Slug pattern: should use variable/config, not hardcoded "dentnotion"
grep -rn '"dentnotion"\|dentnotion_slug\|slug.*dentnotion' \
...
KVKK consent check (dentnotion data — does any workspace data contain personal health identifiers?):
```bash
# Check inbox data for PII patterns (patient names, TC No, phone)
grep -rn "\b[0-9]\{11\}\b\|hasta.*ad[ıi]\|patient.*name\|T\.C\." \
  /Users/apple/Documents/platinum-seo-workspace/projects/dentnotion/inbox/ \
