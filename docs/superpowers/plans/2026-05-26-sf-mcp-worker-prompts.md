...
## Tasks (sequential)
1. **Live run on demo-furniture** — `/pseo-sf-crawl demo-furniture` (operator-facing OR `python3 -c "from scripts.ingestion.sf_crawl_orchestrator import main; main(...)"` directly). Capture output to `/tmp/v1.8-demo-furniture-smoke.log`. ⚠️ This requires SF GUI running with MCP server started.
...
3. **Verify AC-13** — tech-audit `use_sf_mcp_live=True` on demo-furniture; compare rowcount(live) vs rowcount(file-only); record to PHASE_STATUS.md
