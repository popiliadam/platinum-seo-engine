...
>
> **Proje:** Vento (GSC doğrulanmış). **Ortamlar:** VSCode · Claude Mac app · CLI.
...
| 0.2 | Vento'yu aktif yap: `/pseo-active vento` | Aktif marker `vento`'ya geçer (`shared/active.json`) | ☐ |
| 0.3 | Portföyde Vento var mı: `/pseo-status-portfolio` | Triage tablosunda `vento` satırı görünür (durumu ne olursa olsun) | ☐ |
...
| # | Yap | Ne görmelisin | ✅/❌ |
|---|---|---|---|
| 1.1 | Yeni bir oturum aç (VSCode) | SessionStart banner'ı bağlı projeyi (vento) doğru gösterir; başka projenin verisine yazmaz | ☐ |
...
| # | Yap | Ne görmelisin | ✅/❌ |
|---|---|---|---|
| 2.1 | `/pseo-run monthly vento` | Sıralı pipeline çalışır: gsc-pull → quick-wins + content-decay → monthly-report. Model MCP işini yapar, **her adımın çıktısı doğrulanır** (kimlik+içerik geçidi) | ☐ |
| 2.2 | Çıktıyı izle | Bir **coverage kaydı** yazılır (`projects/vento/_state/coverage/<run_id>.json`); verdict `pass` ya da `incomplete` | ☐ |
...
| `/pseo-run monthly vento` çalışır (2.1) | ☐ | ☐ | ☐ |
...
```
/pseo-active vento                 # 0.2  Vento'yu aktif yap
/pseo-status                       # 1.1  bağlama + bekleyen run'lar
/pseo-run monthly vento            # 2.1  çekirdek döngü
