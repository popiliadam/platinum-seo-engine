...
| **GAP-A-B3** (backlinks) | ⏸️ **DEFERRED — operator ruling 2026-06-10: "şimdilik askıda, daha sonra ekleyeceğiz"** (the $100/mo DFS Backlinks API commitment decision is postponed, not declined). Spec stays shelf-ready in the acquisition doc; R-142/R-143 stay RESERVED for it (no other batch may take those ids). When re-opened: dispatch LAST among monthly_report.py touchers + count bump from then-current values + DURUR #0 (first live 4xx → STOP). | acquisition spec **GAP-A1** | — |
