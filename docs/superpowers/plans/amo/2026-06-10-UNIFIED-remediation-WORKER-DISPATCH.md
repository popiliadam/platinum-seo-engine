...
1. **L1 —** `dfs-pull` defaults `location_code=2792`/`language_code="tr"` (SKILL.md:30-39). Remove engine-level country defaults: resolve location/language from `project.config.json` (`market` + `language.content_locale`) via an explicit mapping table in the SKILL body (TR→2792/tr, CA→2124/en, NG→2434/en — verify the DFS location codes against `schemas/dataforseo-endpoint-mapping.schema.json` or existing fixtures before writing them; if unverifiable, the SKILL must instruct resolving via `serp_locations` lookup at run time). Config locale missing → DURUR (never silently default to any country).
