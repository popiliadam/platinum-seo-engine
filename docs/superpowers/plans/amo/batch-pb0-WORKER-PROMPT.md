...
- **Genuine requests (MUST stay Tier-1, status `declared`, byte-identical voice to today):**
  - `"vento'da aylık bakım yap"` → `declared` + the exact `➤ Niyet: aylık bakım algılandı → çalıştır: /pseo-run monthly vento` voice (assert the voice string is UNCHANGED from current behaviour).
  - `"/pseo-run monthly vento"` → `declared`.
