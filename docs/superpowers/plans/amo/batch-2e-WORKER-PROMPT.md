...
> `python -c` writes the same file WITHOUT touching the Write tool (the live demo-furniture `vcc-` pattern). 2e is the
...
invokes Write/Edit (this is the REAL, USED demo-furniture `vcc-` pattern: the engine's R-61 hook blocks `vcc-` via Write,
