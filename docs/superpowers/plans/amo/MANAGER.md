...
| **4 (FAZ)** | Portfolio fan-out + cost/quota ledger + kill-switch + `/pseo-status --portfolio` + recovery runbook; scheduler default OFF. **Manager decomposition: 4a ledger → 4b fan-out → 4c status → 4d scheduler → 4e runbook → D11 live-acceptance.** | 🚧 IN PROGRESS | 0-3 ✅ |
...
| 4f | **Cross-cutting consolidation** (manager SPLIT from spec-4e): `ACTIVE_PROJECTS_MAX` (4 hand-copied literals in reporting modules) → ONE module `scripts/state/active_projects.py` sourced from `portfolio-config.schema` `active_projects.maxItems`; behavior-preserving (value stays 12) + a no-second-definition guard. No schema/command → NO D10. OPTIONAL hygiene (doesn't gate D11). | 📝 **PROMPT AUTHORED** (`d2ad464`) — equivalence-proof refactor | 0-3 ✅ |
