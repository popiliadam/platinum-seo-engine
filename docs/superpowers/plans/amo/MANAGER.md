...
| 2f | **Gate hardening** (SPLIT from spec-2b + 2b QA findings): (a) **2b fail-closed completeness** — gated path must DENY (not fail-open via main's except) when `has_session_consent` RAISES on a corrupt `consent.jsonl` + surface a corrupt-ledger alert; (b) **compound-command segment-split** — catch `cd x && git push` / `ls && curl -X POST` / `git -C /p push` (leading-token misses these); (c) secret-bytes scan (literal pending bytes incl. gitignored) + (d) `dfs_oversized` gate + (e) drift F-rule (every declared outward MCP tool ⊆ a gate matcher). | pending | 2b ✅ |
