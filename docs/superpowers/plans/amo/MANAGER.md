...
| 0d | **Audit H1 fix** (review's highest-value Phase-0 change): extract the PostToolUse inline-python audit into a testable `scripts/hooks/audit_post_tool_use.py` that attributes events to the **session-bound** project (marker → active.json fallback) + classify RUNTIME + tests. Stops silent cross-project `events.jsonl` corruption under multi-window. | **PROMPT READY** → dispatch | 0c |
