...
| 4g | **Self-upgrade versioning** (spec §8, manager SPLIT from spec-4e; OPTIONAL/deferrable like 3-O4): stamp `workflows/*.json` + generated artifacts with engine version; orchestrator asserts `artifact_version == plugin.json version` at run start (fail-loud); add to the `version_bump` 5-file set. Riskier (spine-adjacent run-start + version_bump set + every workflow JSON) → its own focused batch. | pending (author on Süleyman's request; recommend post-D11) | 4a-4d |
