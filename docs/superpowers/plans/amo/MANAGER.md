...
| **4 (FAZ)** | Portfolio fan-out + cost/quota ledger + kill-switch + `/pseo-status --portfolio` + recovery runbook; scheduler default OFF. **Manager decomposition: 4a ledger → 4b fan-out → 4c status → 4d scheduler → 4e runbook → 4f consolidation → (4g opt) → D11 live-acceptance.** | ✅ **BUILD COMPLETE + LIVE-PROVEN** (4a-4f DONE; **D11 ✅ live-acceptance PASSED** on demo-furniture, `2a00049`, suite **2312/0**; 4g OPTIONAL/deferred). | 0-3 ✅ |
| **D11** | Comprehensive all-phases LIVE acceptance (Süleyman ran a live Opus worker on real demo-furniture GSC). First live run of the AMO loop. | ✅ **DONE + manager-verified** (`2a00049`, HEAD=origin/main, suite **2312/0**, cache==dev synced): the live run surfaced **4 real bugs the 2312-test suite never caught** (all "stub agrees with buggy code"), all fixed. **Manager INDEPENDENTLY re-derived each:** (1) `portfolio_runner` read non-canonical `projects` — live `shared/portfolio.json` + `portfolio-config.schema#/required` use **`active_projects`** (11 projects verified) → Faz-4 was fully blocked; **ROOT CAUSE = manager's 4b CONFIRMED-FACTS error** (`projects`), which my 4c "independent" re-derivation INHERITED (seeded `projects`) so it couldn't catch it — only live data did (the raison d'être of D11). (2) `portfolio_writer` wrote `projects` too (latent /pseo-init crash) → aligned. (3) installed plugin-cache was stale (live cmds run from cache) → re-synced (verified cache==dev). (4) `gsc_pull`+`quick_wins` were code_verified but genuinely AGGREGATE (gsc_pull `_aggregate_window` per-page [src-confirmed]; quick_wins = position-band detector) → silent-skip count false-FAILED them on live volumes (500→55, 147→38) → reclassified `model_attested` (identity/freshness/truncation STILL hard-fail; oracle still backstops; content_decay KEPT code_verified as the completeness anchor; D15-precedented; NECESSARY for live). Test updates were coherent + STRONGER (precise per-step classification asserted, none deleted). Live monthly demo-furniture → verdict=pass; consent gate blocked the worker's real rm+git-push → /pseo-approve → passed. ⚠️ Operator follow-up: example cost-ceilings/estimates give daily projection 550 > gsc_calls 500 → set REAL quotas before arming. **🎯 D11 finding reinforces 4g (self-upgrade versioning) — it systematically fixes the plugin-cache-stale class.** | 4a-4f ✅ |
| 4a | **Cost/quota ledger SUBSTRATE** (Faz-4 foundation, like 2a): NEW `schemas/cost-ledger.schema.json` + `scripts/state/cost_ledger.py` — `shared/`-level append-only hash-chained ledger; **atomic reserve-then-confirm under a HARD ceiling** (reserve = read+verify+replay+ceiling-check+append all under ONE `flock(LOCK_EX)` → no parallel overspend; `==`ceiling allowed, `>` raises `CostCeilingExceeded` + writes nothing); confirm(≤reserved)/release; usage=Σ effective-per-reservation by opaque clock-free `period`; resources gsc_calls/dfs_credits/image_spend; ceilings from operator `shared/cost-ceilings.json` (O5). Mirrors consent_ledger (replicated not imported); never os.replace; clock-free; fail-closed on tamper. | ✅ **DONE + GREEN** (`29355a3` prompt + `747fab9` feat, manager-verified EXTRA-HARD **2217/0**, +18 test): **INDEPENDENT atomicity re-derivation** (own thread race, different primitive than worker's subprocesses → exactly 5/20 fit ceiling=50, usage exactly 50, NO overspend); reserve critical section + fail-closed + never-os.replace + clock-free (`reservation_id=f"r{seq}"`) reviewed line-by-line; **manager applied D10** (*.schema.json 24→25 [test rename+assert+lineage] + marketplace "25→26 schemas"; plugin.json unchanged). | 0-3 ✅ |
| 4b | `/pseo-run-portfolio` SEQUENTIAL sweep (Option A, Süleyman) + per-project NON-BLOCKING lock (`project_lock.py`, skip-if-busy) + job-level budget PREFLIGHT (reserve via 4a) + kill-switch (CostCeilingExceeded → release partials [no leak] + `paused` + STOP sweep). PURE coordinator via injectable `run_project_fn` (commit_fn pattern). | ✅ **DONE + GREEN** (`e11d310` prompt + `b4a8582` feat, manager-verified EXTRA-HARD **2241/0**, +24 test): **INDEPENDENT kill-switch re-derivation** (own sweep → paused+not_run+stopped, project-1 confirmed, project-2 partial RELEASED, ledger no-leak/no-overspend) + lock skip-if-held proven; clock-free + never-os.replace + non-blocking reviewed line-by-line; **manager applied D10** (commands 21→22: plugin "22 slash command" + marketplace "22 commands"). ⚠️ **2 follow-ups:** (a) **4d MUST gate scheduler-arming on all-ceilings-set** (O5) — 4b's unset-ceiling→∞ is OK for MANUAL runs but uncapped for autonomy; (b) interactive recipe holds the per-project lock only at the per-project BOUNDARY (a flock can't survive across model turns) — full continuous hold is the pure/autonomous path; the shared 4a ledger stays atomic so budget can't doubly overspend (documented). | 4a ✅ |
| 4c | `/pseo-status --portfolio` triage table (run_id, status, missing_steps, external vs internal; reads coverage + 4a usage). SEPARATE command `commands/pseo-status-portfolio.md` (mirrors 4b); external-vs-internal derived from the coverage `verdict` alone (paused=external, failed/incomplete=internal). NEW command → D10 22→23. | ✅ **DONE + GREEN** (`fe57a65`, manager-verified **2263/0** [+22], prompt `85cd788`): READ-ONLY triage `scripts/reporting/portfolio_status.py` (`latest_coverage`/`classify`/`build_triage`/`render_triage`) — **INDEPENDENT re-derivation** (own tmp ws + own expected values: all 5 verdict classifications + budget math 147/200/53 + unset→None + a TAMPERED-chain no-crash proof + a corrupt-coverage-skip via own os.utime); READ-ONLY proven by own grep (0 write primitives); clock-free (period passed in); reuses `list_projects`+`coverage_path` (no drift). Worker's 3 judgment calls all STRENGTHEN (guarded `read_ceiling` too · mtime tie-break `str(...or stem)` fallback · `paused` hint omits re-run = resumes externally). **Manager applied D10** (22→23: plugin "23 slash command" + marketplace "23 commands"). `pseo-status.md` byte-unchanged. | 4a ✅ |
| 4d | Scheduler **default OFF** + O5 fail-closed arming gate + per-cadence consent + projected daily cost (iterates the ACTUAL on-disk project count). **Design (manager):** NO daemon → an armed-config marker `shared/schedule.json` + `schemas/schedule.schema.json` + `scripts/state/schedule.py` (`arm` REFUSES if any ceiling unset — the load-bearing teeth; 4b follow-up (a)/O5) + `commands/pseo-schedule.md` (arm/disarm/status). Firing is EXTERNAL (cron→`/pseo-run-portfolio`, documented in 4e). Stays disarmed until D11. 2 D10 bumps (schema+command). | ✅ **DONE + GREEN** (`888f71d`, manager-verified **2293/0** [+25], prompt `d2ad464`): `scripts/state/schedule.py` + `schemas/schedule.schema.json` (if/then: armed ⇒ workflow+cadence+consent_ack[const true]+projected_daily_cost+armed_at) + `commands/pseo-schedule.md`. **O5 teeth INDEPENDENTLY re-derived** (own tmp ws + own file-existence assertion: arm-refused→`schedule.json` NEVER created for unset/zero ceilings, `.missing` exact; consent_ack=False AND =1 strict-identity refused→nothing written; full arm projected_daily_cost {30,15,6}=estimate×3proj×cadence + weekly ×1/7; default-OFF no-file; disarm idempotent+disarmed_at; schema if/then all 7 cases). `arm` order = enum→consent→**O5 gate→write** so refuse≡write-nothing (same path); `arm` re-runs `all_ceilings_set` itself (fail-closed even if a ceiling deleted mid-flight). Clock-free; marker via `_atomic_write_json` (os.replace, mutable pointer). **Manager applied D10** (3 edits: `*.schema.json` 25→26 [test rename `..._twenty_five`→`..._twenty_six`+assert+lineage] · all-`*.json` 26→27 marketplace "27 schemas" · commands 23→24 plugin "24 slash command"+marketplace "24 commands"). Frozen consent.schema/spine/drivers/ledger/`/pseo-run-portfolio` untouched; fires nothing. | 4a,4b |
| 4e | Recovery runbook (DOC) `docs/RUNBOOK-portfolio-recovery.md` — symptom→what-you-see→exact-recovery for kill-switch/skip/fail/ledger-break/scheduler + the external-trigger safety order; + a citation-existence guard test. No runtime change → NO D10. **Dispatch AFTER 4d merges** (documents the real shipped surface). | ✅ **DONE + GREEN** (`e09ac2f`, manager-verified **2298/0** [+5 guard tests], prompt `d2ad464`): `docs/RUNBOOK-portfolio-recovery.md` (7 sections, every operator surface quoted VERBATIM from shipped source) + `tests/docs/test_runbook_citations.py` (citation-existence guard, slash + `-m` module forms, teeth test catches a dangling cite). **Manager INDEPENDENT verification** (own citation re-extraction: all 6 commands + 5 scripts resolve; spot-grepped the quoted `/pseo-schedule` status/consent/disarm messages + `--resume` are REAL in source = NOT invented; guard teeth confirmed). Scope = 2 new files, 0 modified → NO D10; shipped code/commands/schemas byte-unchanged. Closes the forward-reference 4b/4d already make to "4e recovery runbook". | 4a-4d |
| 4f | **Cross-cutting consolidation** (manager SPLIT from spec-4e): `ACTIVE_PROJECTS_MAX` (4 hand-copied literals in reporting modules) → ONE module `scripts/state/active_projects.py` sourced from `portfolio-config.schema` `active_projects.maxItems`; behavior-preserving (value stays 12) + a no-second-definition guard. No schema/command → NO D10. OPTIONAL hygiene (doesn't gate D11). | ✅ **DONE + GREEN** (`485b2f2`, manager-verified **2312/0** [+14], prompt `d2ad464`): NEW `scripts/state/active_projects.py` reads `#/properties/active_projects/maxItems` at import (fail-loud `ActiveProjectsMaxError`, never silent-12); **5 copies** swapped to import (worker caught a **5th** — `portfolio_task_heatmap.py` used annotated `: int = 12`, my hand-off said 4; guard regex spans both forms). **Manager INDEPENDENT equivalence** (imported all 5 consumers + source → all resolve 12==schema; fail-loud proven on 4 bad-schema cases; diff = pure literal→import, ZERO use-site/logic change, ZERO tests edited). **ADR-027 ≤600-line ceiling** on `portfolio_kpi_trend.py` (599→600) resolved NET-ZERO (import tight after sys.path bootstrap, inline comment) WITHOUT editing the ceiling test or changing behavior — correct DURUR discipline. No D10. | 0-3 ✅ |
| 4g | **Self-upgrade versioning** (spec §8, manager SPLIT from spec-4e; OPTIONAL/deferrable like 3-O4): stamp `workflows/*.json` + generated artifacts with engine version; orchestrator asserts `artifact_version == plugin.json version` at run start (fail-loud); add to the `version_bump` 5-file set. Riskier (spine-adjacent run-start) → its own focused batch. **Scoped (manager 2026-06-09):** the stamp is ALREADY plumbed (`engine_version` threaded through `run_workflow`→`coverage.build_record`, schema-declared) but always None → 4g (a) POPULATES it from `plugin.json` version + (b) adds a run-start RESUME version-mismatch assertion (un-stamped prior allowed; fresh run byte-identical). NOT the cache-stale fix (version-keyed install handles that) — orthogonal stale-DATA-artifact guard. | ✅ **DONE + GREEN** (`649e5fa`, manager-verified **2327/0** [+15], prompt `batch-4g-WORKER-PROMPT.md`): NEW `scripts/state/engine_version.py` (plugin.json `#/version` single source, fail-loud, parents[2] — mirrors active_projects.py); `run_workflow` POPULATES the stamp (caller-explicit wins) + a run-start RESUME guard `_assert_no_version_drift` raising `EngineVersionMismatch` (reads-before-write → mismatch aborts ZERO-side-effect, prior preserved); content_pipeline stamps too (assert is run_workflow-only). **Manager INDEPENDENT teeth** (own tmp: 1.9.5 prior + 2.0.0 run → raised + both versions named + prior NOT overwritten; back-compat un-stamped→no-raise; same-version/fresh/dry-run→no-raise). Behavior-preserving for fresh runs (no fixture edits). Spine/schema/commands untouched; no D10. **Open (optional 4h):** extend the resume-guard to content_pipeline for symmetry. Lands on main post-v2.0.0 (no bump yet; next version bump activates its cross-version guard). | 4a-4d |
