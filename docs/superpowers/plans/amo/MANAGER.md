...
| **4 (FAZ)** | Portfolio fan-out + cost/quota ledger + kill-switch + `/pseo-status --portfolio` + recovery runbook; scheduler default OFF. **Manager decomposition: 4a ledger → 4b fan-out → 4c status → 4d scheduler → 4e runbook → 4f consolidation → (4g opt) → D11 live-acceptance.** | ✅ **BUILD COMPLETE** (4a-4f all DONE+pushed, suite **2312/0**; 4g self-upgrade-versioning OPTIONAL/deferred). **Only D11 (Süleyman's comprehensive live-acceptance) remains to close the AMO build.** | 0-3 ✅ |
