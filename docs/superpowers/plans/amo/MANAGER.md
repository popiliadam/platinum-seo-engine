...
| 4c | `/pseo-status --portfolio` triage table (run_id, status, missing_steps, external vs internal; reads coverage + 4a usage). SEPARATE command `commands/pseo-status-portfolio.md` (mirrors 4b); external-vs-internal derived from the coverage `verdict` alone (paused=external, failed/incomplete=internal). NEW command → D10 22→23. | 📝 **PROMPT AUTHORED** (`85cd788`) — read-only triage module `scripts/reporting/portfolio_status.py` + recipe + tests | 4a ✅ |
