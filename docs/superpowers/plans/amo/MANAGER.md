...
| 4c | `/pseo-status --portfolio` triage table (run_id, status, missing_steps, external vs internal; reads coverage + 4a usage) | pending | 4a ✅ |
| 4d | Scheduler **default OFF** + explicit per-cadence consent + projected daily cost shown before arming (iterates the ACTUAL on-disk project count, not "3-5") | pending | 4a,4b |
| 4e | Recovery runbook (doc) + cross-cutting consolidation (ACTIVE_PROJECTS_MAX 1-module, self-upgrade versioning) | pending | 4a-4d |
