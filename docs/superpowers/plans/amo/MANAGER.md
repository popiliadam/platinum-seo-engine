...
> **🎯 Faz-3 governance line COMPLETE** (secrets + lint2 both shipped). **NEXT: 3-O4 light-promote** (extract
> shared DATA-driver for monthly/audit/setup; content stays separate artifact-driver) → then Faz 4. O4
> surface pre-mapped (3 drivers ~80% identical; only setup→audit `_run_one` coupling to clean; oracle
> depends only on `.STEPS` → unaffected; spine + content untouched).
