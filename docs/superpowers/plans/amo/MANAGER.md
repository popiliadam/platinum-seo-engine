...
> **Faz 1 PROGRESS: 1a ✅ (`7c8d66d`) → 1b ✅ DONE+GREEN (`d045da5`, isolated 46/46). Spec's 1b SPLIT → 1b (machinery) + 1b2 (relocation). 1c (∥) FUNCTIONALLY COMPLETE in-tree but breaks 10 pre-existing UserPromptSubmit tests (by-design "one voice" change) → awaiting 1c worker REPORT + a test-contract migration before commit.**
>
...
| 1c | Intent router (one-voice UserPromptSubmit; marker lifecycle session_id/turn_id/intent_id). `intent_router.py` (Tier-1 canonical→inject `/pseo-run`+`declared` marker; Tier-2→advisory+`superseded`) + `intent-marker.schema.json` + hook cmd-swap (one voice, env_probe kept) + RUNTIME_HOOK_SCRIPTS reg + README. **turn_id router-ASSIGNED (probe-free; per-turn payload id unproven)**; marker rewritten every prompt → "current-turn only" structural. D10: draft 22→23 + marketplace 23→24. | 📝 **PROMPT AUTHORED** (`batch-1c-WORKER-PROMPT.md`); ∥ 1b | 1a ✅ |
| 1b2 | **(blast-radius migration — manager ISOLATED, after 1b proven)** relocate gsc-pull / quick-wins / content-decay master.xlsx writes from SKILL.md prose into `committer.commit` (fixes known gsc `append→replace` snapshot-dup bug per [[feedback_gsc_pull_replace_not_append]]); each skill's existing tests stay green | pending | 1b |
