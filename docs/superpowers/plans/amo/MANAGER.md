...
| 4b | `/pseo-run-portfolio` cross-project fan-out (disjoint per-project locks) + per-step mid-job budget PREFLIGHT (reserve via 4a) + kill-switch (CostCeilingExceeded → run `paused`, not silent degrade) | pending | 4a ✅ |
