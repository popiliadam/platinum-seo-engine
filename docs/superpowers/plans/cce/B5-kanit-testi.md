...
Operatör/manager bir `primary_keyword` + `market`/`locale` + `content_type` verir (örn. aktif proje `bigcat-tr`'den düşük-riskli bir bilgilendirici konu). **Konu verilmeden başlama** — manager'dan iste. (Plugin-agnostik: çıktıda proje slug literal yok.)
