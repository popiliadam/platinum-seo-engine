...
| SF Issue-Name match (keyword) | → sheet | tech_seo issue_category | aluminumstation examples |
|---|---|---|---|
...
**Cross-check requirement:** the implementer MUST add a test asserting all 42 aluminumstation
...
- aluminumstation expect: ~13 summary + 66 URL ≈ 79 rows.
...
- aluminumstation expect: ~8 internal rows (7 /services/ 301 + portfolio 404; exclude
...
- aluminumstation expect: 2 rows (Article 13, BreadcrumbList 45; 0 errors).
...
- aluminumstation expect: 58 rows (in_title 58 / in_meta 26 / in_h1 38 — matches the live consumer).
...
- aluminumstation expect: ~30 rows (granular, 1:1 with SF issues; human had 20 via manual merges).
...
- aluminumstation expect: ~9 rows (6 Security + URL non-ASCII + Pagination + high-crawl-depth).
