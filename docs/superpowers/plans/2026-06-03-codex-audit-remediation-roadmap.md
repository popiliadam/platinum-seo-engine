...
| P0-04 | Workspace configs fail current schemas | ✅ **bigger scope** | **P0** | 8/10 projects stale: 7×`1.3`, 1×`1.4` (demo-fintech); only demo-aluminum-ca + demo-baby-com on `1.5`. `portfolio.json` 10 active vs `portfolio-config.schema.json` maxItems:8 → "is too long". |
...
- **D2 — portfolio cap (maxItems:8).** *Recommendation:* raise to 12 (or make configurable).
  *Why:* the portfolio already runs 10 active projects (demo-baby added 2026-06-03); evidence says
