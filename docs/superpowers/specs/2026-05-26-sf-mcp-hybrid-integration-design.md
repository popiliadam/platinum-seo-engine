...
```
Operator drops CSVs → projects/demo-furniture/sf-exports/2026-05-26/raw/
    ↓
...
Operator runs: python3 scripts/migrations/migration_0005_project_config_1_4_to_1_5.py \
    --in projects/demo-furniture/project.config.json [--out PATH] [--dry-run]
    ↓
...
Audit summary to stderr (per 0004 pattern)
    ↓
Operator can now /pseo-sf-crawl demo-furniture (opt-in by setting mcp.enabled=true in config or via /pseo-sf-status)
```
...
| **Phase 7** — Pilot smoke + release | (1) Live run on demo-furniture end-to-end; (2) record AC-10 + AC-13 evidence to PHASE_STATUS.md; (3) drift-check 27 invariants re-run GREEN (AC-17); (4) schema-validate full sweep GREEN (AC-18); (5) full pytest baseline GREEN (target ~1210 PASS / 12 SKIPPED / 0 FAIL — AC-16); (6) git tag v1.8.0 annotated; (7) **NEW: rollback drill — verify operator can revert via `git revert {commit-range}` + Migration 0005 reverse runs cleanly + 1184 v1.7 baseline restored** | **0.75 day** | LOW |
...
10. ✅ End-to-end smoke (manual + recorded in `docs/PHASE_STATUS.md`): operator runs `/pseo-sf-crawl demo-furniture`; verified by (a) `ls projects/demo-furniture/sf-exports/$(date +%Y-%m-%d)/raw/*.csv` returns ≥14 CSVs, (b) `python3 -c "from openpyxl import load_workbook; print([s for s in load_workbook('projects/demo-furniture/master.xlsx').sheetnames if s in {'crawl_sitemap','redirect_404','schema','on_page_audit','tech_seo','robots_txt'}])"` returns all 6 sheet names, (c) `grep -c '"source":{"kind":"sf_mcp"' projects/demo-furniture/_state/events.jsonl` returns ≥1
11. ✅ Operator can disable orchestrator entirely: with `sf.mcp.enabled=false` in project.config.json, manual CSV drop + `python3 scripts/ingestion/sf_import.py --project demo-furniture --sf-export-path projects/demo-furniture/sf-exports/{date}/` succeeds with same exit code (0) and same 6-sheet outputs as v1.7 baseline; verified by `pytest tests/skills/test_sf_import.py -v` (existing 6 cases stay GREEN)
...
13. ✅ tech-audit (selected as representative consumer per Phase 5) tested live with `use_sf_mcp_live=True` on demo-furniture; operator confirms `master.xlsx#tech_seo` rowcount(live) ≥ rowcount(file-only baseline); evidence recorded in `docs/PHASE_STATUS.md` v1.8 closeout entry. CI test remains mock-only.
...
| **Q-SF-MCP-03** Default max_wait_minutes for crawl polling | 180 (3h) | 360 (6h) for demo-petcare-scale 30K+ URLs |
