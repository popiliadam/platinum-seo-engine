...
| OQ-FILEPATH-EXPORTS | **Task B = CONFIRMED BUG (not just verify):** orchestrator ✅ (build_export_plan + file_path, `a714e43`) + tech-audit ✅ (AC-13 `6ee6fb9`: file_path export). **3 consumers STILL broken** (schema-audit, on-page-audit, internal-links): live-mode prose uses `call_tool("sf_generate_report", report_name=, save_report=False)` + `response.get("truncated"/"rows")` — none exist; real SF REJECTS inline >100KB (all_inlinks 2.1MB would FAIL). Apply the AC-13 pattern (dispatch + file_path + load_crawl + utf-8-sig + transform). | **P2** (3 consumers remain) |
