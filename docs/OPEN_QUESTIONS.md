...
| OQ-SHEET-PROJECTION | AC-10 chain: `scripts.ingestion.sf_import` does Tier-validation + envelope ONLY; the per-sheet projection (raw CSV → master.xlsx 6 SF sheets) is sf-import **SKILL.md Step 6 prose** (transaction.append) = stub-mod. After a live sf_import the 6 sheets are UNCHANGED (82→82 etc.). To complete AC-10 "6 sheets repopulated" the projection must run (interpreter-level) OR be scripted. | **P2** (Task C remaining) |
