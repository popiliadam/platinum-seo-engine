...
### Q-V1.9.2-SF-MCP-RETRY-BUSY-01: SF MCP retry-on-busy fix + Spider-state RESOLVED; 23/29 tools live-proven [P2] 🩹 FIX LANDED + ✅ PUSHED origin/main (c6c0268+65c5c52) 2026-06-02
...
**Live tool coverage (23/29 confirmed FUNCTIONAL on real data — demo-aluminum, 1822 URLs):** ✅ list_crawls, list_allowed_base_directory, list_available_reports, list_available_bulk_exports, list_directories(relative), list_available_filters, load_crawl, crawl_progress, generate_report (real Crawl Overview), url_info (real SEO data), url_links, get_url_screenshot, exports (export_seo_element_urls + bulk_export_page_content), file-ops (read/list/copy/move, relative-path), **run_node_js_script ✅, npm_install ✅** (SF Node.js Runtime enabled). ⚠️ expected-limit (work, hit cap → use file_path/smaller): url_content (8192-char single-page cap), generate_bulk_export (3.6MB>100KB→file_path). **6/29 remaining = destructive/intrusive, operator-gated (Task A controlled sweep on example.com):** sf_crawl, sf_pause_crawl, sf_resume_crawl, sf_clear_crawl, sf_open_url_in_browser, sf_export_embeddings.
...
| OQ-LOADCRAWL-TIMEOUT | `sf_load_crawl` can TIME OUT under heavy load (esp. after big exports) — v1.9.1 client raises clean `SfMcpTimeoutError` (no hang); light reads (health/list_crawls) stay instant. Mitigation now: operator restarts SF to clear. Candidate v1.9.3: orchestrator-level retry/extend-on-timeout for sf_load_crawl. | **P2** (Task D) |
| OQ-REMAINING-SWEEP | 6 tools not yet exercised live = DESTRUCTIVE/intrusive (sf_crawl, sf_pause_crawl, sf_resume_crawl, sf_clear_crawl, sf_open_url_in_browser, sf_export_embeddings) — operator-gated (Task A on example.com). [run_node_js_script + npm_install now PROVEN ✅] | P2 |
