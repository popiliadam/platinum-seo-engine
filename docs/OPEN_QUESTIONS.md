...
| OQ-LOADCRAWL-TIMEOUT | `sf_load_crawl` can TIME OUT under heavy load — **CORROBORATED 2026-06-02**: load_crawl(bayder, 160 URLs) timed out right after the Task A destructive sweep (SF degraded). Client did its built-in **3-attempt** timeout-retry (each up to `timeout_seconds`) then raised clean `SfMcpTimeoutError` (no hang); export_crawl/url_info still worked (crawl loaded server-side). ⚠️ 3×per-request-timeout can total MINUTES (120s→~6min). Mitigation now: operator restarts SF. v1.9.3 (Task D): orchestrator-level shorter per-request timeout + retry-with-restart-prompt for sf_load_crawl. | **P2** (Task D) |
