...
### Q-V1.9.2-SF-MCP-RETRY-BUSY-01: SF MCP retry-on-busy fix + Spider-state RESOLVED; 29/29 tools live-exercised (Task A complete) [P2] 🩹 FIX LANDED + ✅ PUSHED origin/main (c6c0268+65c5c52) 2026-06-02
...
**Live tool coverage — 29/29 ALL tools live-exercised (Task A COMPLETE 2026-06-02):** prior 23 (demo-aluminum 1822 URLs) + Task A destructive sweep on example.com (throwaway) + demo-ngo completeness probe. **✅ all 6 previously-remaining destructive/intrusive tools now PROVEN:** sf_crawl ("crawl succesfully started") → sf_crawl_progress (**caught running at 50%, active:1**) → sf_pause_crawl ("Crawl has paused", frozen at 50%) → sf_resume_crawl ("resumed" → 100%/4 URLs) → sf_clear_crawl ("Crawl has been cleared", test crawl ONLY) + sf_open_url_in_browser ("opened in external browser") + sf_export_embeddings (callable; expected domain error "No embeddings configured"). **✅ file-ops first-hand** (real tools are read/write/create_directory — NOT copy/move which don't exist): sf_create_directory + sf_write_text_file + sf_read_text_file (round-trip) + sf_export_crawl (exported) + sf_list_available_filters_for_seo_element + sf_list_available_data_fields_for_seo_element_and_filter. ✅ also proven prior: list_crawls, list_allowed_base_directory, list_available_reports, list_available_bulk_exports, list_directories, generate_report, url_info, url_links, get_url_screenshot, export_seo_element_urls, bulk_export_page_content, generate_bulk_export, run_node_js_script, npm_install. ⚠️ expected-limit (use file_path): url_content (8192-char single-page cap), generate_bulk_export (>100KB→file_path). **2 'ERR' in the sweep = CORRECT domain behavior, NOT transport/Spider-state:** embeddings-not-configured + pause-a-finished-crawl ("UiInactiveState"). retry-on-busy surgical filter re-validated: the UiInactiveState error did NOT match `_is_spider_busy` → returned clean with no futile retry. **0/29 remaining.** Sandbox residuals (no SF delete tool): `/Users/apple/seo_spider_mcp_server/pseo_taskA_probe/{probe.txt,demo-ngo_export.csv}` (harmless, clearly labeled).
...
| ~~OQ-FILEPATH-EXPORTS~~ | ✅ **RESOLVED 2026-06-02 (`c193ec0`):** ALL 4 consumer skills + orchestrator now use `file_path` (resolves the >100KB inline cap). orchestrator (`a714e43`) + tech-audit AC-13 (`6ee6fb9`) + internal-links/schema-audit/on-page-audit (`c193ec0`, AC-13 pattern: dispatch + file_path + load_crawl + utf-8-sig[native CSV]/ndjson_to_csv[seo-element]). Live-proven each. **2 live-caught bugs fixed along the way:** AC-13 BOM (utf-8-sig + transform `_clean_key`) + on-page single-export (page_titles has no meta/h1 cols → export 3 + merge per-URL). | ~~P2~~ DONE |
| ~~OQ-LOADCRAWL-TIMEOUT~~ | ✅ **RESOLVED 2026-06-02 (Task D, commit `9d662b5`):** `SfMcpClient.load_crawl(crawl_id)` fires the load FAST (single short attempt via `_timeout_override`+`_max_attempts_override=1`, tolerating the client-side timeout) then polls `sf_crawl_progress` until `_progress_indicates_loaded` (active==0 + completed>0/100%). **Live-proven: demo-aluminum (1822) loads in 15.1s** (was 180s-then-raise); post-load url_info returns real data. +12 tests, 21 existing unchanged. Wiring into orchestrator-resume + consumer-live-mode = follow-up (those flows are stub-mod; AC-13 will wire it for tech-audit). | ~~P2~~ DONE |
| ~~OQ-REMAINING-SWEEP~~ | ✅ **RESOLVED 2026-06-02 (Task A):** all 6 destructive/intrusive tools (sf_crawl, sf_pause_crawl, sf_resume_crawl, sf_clear_crawl, sf_open_url_in_browser, sf_export_embeddings) live-exercised on example.com → **29/29 complete**. | ~~P2~~ DONE |
| ~~OQ-SHEET-PROJECTION~~ | ✅ **RESOLVED 2026-06-02 (v1.9.4: `6db5551`+`294f0db`+`cb6dc0b`+`9cf5dd3`+`000ed40`):** built `scripts/ingestion/sf_projection.py` (6 pure BOM-safe mappers) + `scripts/util/sf_issue_taxonomy.py` (routing SSOT) + `transaction.replace` (idempotent refresh — was MISSING, NOT pre-existing as the hand-off assumed) + wired sf_import Step 6/7 (projection→replace + sf_csv provenance). **Enum solved by ROUTING, no schema change** (ADR-028 honored; SF non-fitting issues → robots_txt/redirect_404; Issue Name in detail → zero data loss; evidence = 2026-05-14 human import). **LIVE-PROVEN on demo-aluminum:** 6 sheets repopulated (74/8/2/58/30/9), 0 schema errors, idempotent (2nd run identical), sf_csv provenance, drift-check 23/24 PASS (F-08 crawl_sitemap invariant PASS). Independent Manager live cross-check caught 2 latent bugs 1427 mock tests missed (no replace mode → dup; `_heading_findings` "Headings" out-of-enum → RowSchemaError). pytest 1338→1427/11/0. | ~~P2~~ DONE |
| ~~OQ-SMOKE-HEALTH-PROBE~~ | ✅ **RESOLVED 2026-06-02 (commit `7a93ff7`):** smoke liveness probe → `SfMcpClient(...).health()` handshake. Running it live then exposed a latent shape bug (test expected `{allowed_directory\|value}` but real SF returns the path in `content[*].text`) — fixed the assertion to the real MCP envelope + isError guard. Smoke now RUNS+PASSES vs SF 24 (1325 PASS/11 SKIP local; 1324/12 on CI). Subsumes the old OQ-SF-SMOKE-COMMENT. | ~~P3~~ DONE |
...
| OQ-V1.9-P1-W-02 | spec Scenario 4 (lines 664-665, 673) invoked `migration_0005 --project demo-furniture [--dry-run] [--no-backup]` — but real argparse is `--in`/`--out`/`--dry-run` (NO `--project`/`--no-backup`). R-10 codified the CORRECT shape; spec Scenario 4 was the stale side. | LOW | ✅ FIXED Phase 1 — Fix Worker changed to `--in projects/demo-furniture/project.config.json [--out PATH] [--dry-run]` + backup note to "in-place mode (default; omit --out)". sf_import.py `--project` refs (spec:579/616/884) left intact (correct — different script). |
| OQ-V1.9-P1-W-03 | `docs/superpowers/plans/2026-05-26-sf-mcp-worker-prompts.md:599` (v1.8 Phase 7 rollback-drill task #7) describes `migration_0005 --project demo-furniture --reverse` — `--reverse` does not exist (forward-only 1.4→1.5 script); line already hedged "(or equivalent rollback if migration script supports)". | LOW | ⏳ **DEFERRED to v1.10 retro** — methodology reword (the real v1.8 drill used `git reset --hard`; v1.9 PROMPT 7 already prescribes git-reset correctly). NOT a clean flag-swap like W-01/W-02; in a shipped historical file; zero runtime impact. Manager boundary: Phase 1 fixes verified flag-swaps of R-2/R-10 class, defers methodology rewords (scope-discipline per v1.8 "dispatch cascade over-specification" lesson). |
...
- **Q-SF-MCP-03** Default max_wait_minutes for crawl polling → **180** (3h; default; Q-SF-MCP-11 tuned together). DEFERRED to v1.1+ for demo-petcare-scale 30K+ URLs (360).
...
**Blocking Phase:** None for v1.3.x maintenance (demo-hvac workaround tested, demo-dental intact); v1.4 milestone scope (bootstrap modernize + path convention codify).
**Cross-refs:** demo-hvac workaround atomic kanıt (workflow `demo-hvac-2026-05-07-8c5b`, master.xlsx SHA-256 `f18fc6f6ffde3387b9349441bbff3d6301a0d6b9a6d03af7c383ddf40bc9f3f6`); skills/meta/init-project/SKILL.md §Step 4 (subprocess CLI invoke); schemas/project-config.schema.json v1.3 (paths field schema); rules/single-source-of-truth.md (workspace path convention authority).
...
**Options:**
- a) Update demo-dental project.config.json to use canonical keys: `pronoun_preference: "siz"`, `formality: "formal"`
