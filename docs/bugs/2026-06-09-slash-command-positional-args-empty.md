...
parametresi olarak görüp boş string'e genişletiyor. Sonuç: kullanıcı `/pseo-bind dentnotion` yazsa bile
alttaki CLI `session_binding bind ""` olarak çağrılıyor ve `invalid slug (… ''):` hatası dönüyor.

## Tekrar üretme (REPRO)

1. `/pseo-bind dentnotion` çalıştır.
2. Beklenen: `bound session <id> → dentnotion`.
3. Gerçekleşen:
   ```
   ...
   ```
   Dikkat: argüman `dentnotion` verilmesine rağmen komut `bind ""` (boş) olarak genişledi; `$2 $3` de boşa düştü.
...
- Kanıt: aynı repoda `$ARGUMENTS` kullanan komutlar (`pseo-init`, `pseo-cannibalization`) çalışıyor;
  `$1` kullanan bloklar boş alıyor. CLI'nin kendisi sağlam — elle `python3 -m scripts.state.session_binding bind dentnotion`
...
1. `/pseo-bind dentnotion` → `bound session <id> → dentnotion` görmeli, exit 0.
2. `/pseo-bind` (argümansız) → `MISSING_SLUG: usage …` görmeli (CLI çağrılmamalı).
3. `/pseo-active vento` ve `/pseo-approve <run> <action> <target>` de gerçek argümanlarla geçmeli.
