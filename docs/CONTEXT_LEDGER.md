...
- **All 6 previously-remaining destructive/intrusive tools PROVEN** (via engine `SfMcpClient`, busy_retry_max=8): sf_crawl ("crawl succesfully started") → sf_crawl_progress (**caught running at 50%, active:1** — example.com had 4 URLs) → sf_pause_crawl ("Crawl has paused"; progress frozen at 50% = genuine pause) → sf_resume_crawl ("resumed" → 100%/completed:4) → sf_clear_crawl ("Crawl has been cleared"; test crawl ONLY — saved demo-aluminum/demo-ngo/demo-fintech untouched) + sf_open_url_in_browser ("opened in the external browser"; visible tab, operator-approved) + sf_export_embeddings (callable; expected domain error "No embeddings have been configured for this crawl").
- **File-ops first-hand** (corrected: real tools are read/write/create_directory — NOT copy/move): sf_create_directory + sf_write_text_file + sf_read_text_file (round-trip exact-content verified). **Completeness probe (demo-ngo):** sf_export_crawl ✅ + sf_url_info ✅ (real SEO data: Response Time 0.459, HTTP Redirect) + sf_list_available_filters_for_seo_element ✅ (All/Missing/Duplicate/Over 60 Chars/…) + sf_list_available_data_fields_for_seo_element_and_filter ✅ (Address/Title 1/Indexability/…).
...
- **⚠️ OQ-LOADCRAWL-TIMEOUT corroborated:** sf_load_crawl(demo-ngo, 160 URLs) timed out right after the destructive sweep (SF in degraded state). Client's built-in 3-attempt timeout-retry (each up to `timeout_seconds`; I used 120s → ~minutes total) then raised clean `SfMcpTimeoutError` (no hang ✅). BUT export_crawl/url_info still returned real demo-ngo data → SF DID load server-side; client just timed out waiting. **Operator should restart SF (Stop→Start MCP Server / relaunch) before Task C heavy crawl ops** to clear the degraded state. Reinforces Task D (v1.9.3): orchestrator-level shorter per-request timeout + retry-with-restart.
...
- **F-16 `.mcp.json` 543B/md5 `93523d4` UNCHANGED; no code changes (Task A = live evidence + doc record only).** Sandbox residuals (no SF delete tool): `/Users/apple/seo_spider_mcp_server/pseo_taskA_probe/{probe.txt,demo-ngo_export.csv}` (harmless).
...
## 🟢 Task C — AC-10 orchestrator export dispatch BUILT + LIVE-PROVEN (2026-06-02, pilot=demo-aluminum, commit a714e43)
- **Operator decisions:** pilot=demo-aluminum (live 1822-URL crawl in SF), scope=AC-10 smoke first → then AC-13 code.
- **Prep:** workspace = SEPARATE repo `/Users/apple/Documents/platinum-seo-workspace` (NOT in engine). Migrated `demo-aluminum-ca/project.config.json` 1.3→1.4→1.5 (Migrations 0004+0005, .bak'd + my own `.pre-task-c-bak`) + set `sf.mcp.enabled=true` + `allowed_directory=/Users/apple/seo_spider_mcp_server` (F-25 now passes). SF allowed base confirmed via sf_list_allowed_base_directory.
...
- **Task D (`9d662b5`):** `SfMcpClient.load_crawl(crawl_id, *, settle_timeout_seconds=180, poll_interval_seconds=3, load_fire_timeout_seconds=15)` — fire the load FAST (single short attempt, tolerate the client-side timeout that large saved crawls reliably cause; the crawl loads server-side anyway) → poll `sf_crawl_progress` until `_progress_indicates_loaded` (not isError + active==0 + completed>0/percentComplete>=100). Added optional keyword-only `_timeout_override` + `_max_attempts_override` to `call_tool` (None = identical default behavior; NOT leaked into JSON-RPC arguments). Worker offline TDD (+12 tests, 21 existing client tests unchanged); Manager **live-validated**: demo-aluminum (1822) loads in **15.1s** (was 180s-then-raise), post-load url_info returns real data. → OQ-LOADCRAWL-TIMEOUT RESOLVED.
...
- **Next session (fresh context):** AC-10 sheet projection (sf-import Step 6) + AC-13 tech-audit live-merge (`live_findings` + `sf_crawl_id` + wire load_crawl) + Task B (file_path ×4 consumers). Workspace repo demo-aluminum config migration is UNCOMMITTED (operator's data repo — decide separately).
...
- **🎯 LIVE-PROVEN E2E (demo-aluminum):** domain-match `demo-aluminum.example`→`fc718e3f` ✓ → load → export 42 real issues → transform → **tech_seo rowcount(live)=42 ≥ rowcount(file)=0** ✓; severities HIGH/MEDIUM/LOW valid. pytest 1325→1332 PASS / 11 SKIP / 0 FAIL (+7: 6 Worker + 1 BOM). F-16 unchanged.
...
- Applied the AC-13 pattern (sf_list_crawls domain-match → load_crawl → SF_EXPORT_DISPATCH → file_path → correct read → transform) to internal-links + schema-audit + on-page-audit (all had tech-audit's broken API). Worker offline TDD + Manager live cross-check on demo-aluminum.
