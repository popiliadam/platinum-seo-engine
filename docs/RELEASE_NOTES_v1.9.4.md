...
- `_live_finding_to_finding` now routes SF Issue Names through `sf_issue_taxonomy` → only valid 5-enum `tech_seo` rows (non-tech_seo issues dropped; they belong to robots_txt/redirect_404). 42 real aluminumstation issues: was 42/42 out-of-enum → now 30 in tech_seo (0 out-of-enum), 12 routed away.
...
## 5. Live cross-check (aluminumstation, 2026-06-02)
...
## 7. Known limitations
- `schema` sheet projection is shallow by nature: raw SF `structured_data_all` exposes only `Type-1` (Article + BreadcrumbList for aluminumstation). The deeper multi-type + "missing schema" gap-analysis remains a DFS/human-enrichment concern (`schema_audit_transform`), not raw-CSV-derivable.
