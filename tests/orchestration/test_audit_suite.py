...
def _rows(n: int) -> list[dict]:
    return [{"q": f"kw{i}", "url": f"https://vento.example/p{i}"} for i in range(n)]
...
def _write_drop(workspace, entry, rows, *, declared_count=None, mtime=NOW - 60,
                run_id=RUN_ID, slug=SLUG, site_url="https://vento.example"):
...
def test_build_steps_pins_site_url_only_for_gsc_step(tmp_path: Path) -> None:
    ...
    cfg.write_text(json.dumps({"gsc": {"site_url": "https://vento.example"}}),
                   encoding="utf-8")
    specs = {s.name: s for s in build_steps(RUN_ID, SLUG, tmp_path)}
    assert specs["cannibalization"].expected_site_url == "https://vento.example"
...
def test_missing_code_verified_step_is_incomplete(tmp_path: Path) -> None:
    ...
    assert rem["one_line_fix_command"] == "/pseo-run audit vento --resume"
...
def test_cli_no_drops_prints_audit_remediation_and_writes_coverage(tmp_path, capsys) -> None:
    ...
    assert "/pseo-run audit vento --resume" in out
