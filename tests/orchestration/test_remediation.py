...
def test_pass_verdict_yields_no_remediation() -> None:
    ...
    assert remediation(record, slug="demo-furniture") is None
...
def test_incomplete_names_missing_step_and_resume_command() -> None:
    ...
    rem = remediation(record, slug="demo-furniture")
...
def test_render_contains_fix_command_and_is_nonempty() -> None:
    ...
    assert out.rstrip().splitlines()[-1].strip().endswith("/pseo-run monthly demo-furniture --resume")
