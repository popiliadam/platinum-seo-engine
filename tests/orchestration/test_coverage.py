...
def test_build_record_full_validates() -> None:
    ...
    rec = coverage.build_record(
        ...
        project_slug="demo-furniture", engine_version="1.9.5",
...
def test_coverage_path_shape(tmp_path: Path) -> None:
    p = coverage.coverage_path(tmp_path, "demo-furniture", RUN_ID)
    assert p == tmp_path / "projects" / "demo-furniture" / "_state" / "coverage" / f"{RUN_ID}.json"
...
def test_write_coverage_writes_valid_record(tmp_path: Path) -> None:
    rec = coverage.build_record(
        ...
        required_satisfied=True, verdict="pass", project_slug="demo-furniture",
    )
    path = coverage.write_coverage(rec, workspace_root=tmp_path, project_slug="demo-furniture", run_id=RUN_ID)
    assert path.exists()
...
def test_write_coverage_rejects_invalid_and_writes_nothing(tmp_path: Path) -> None:
    ...
    with pytest.raises(coverage.CoverageValidationError):
        coverage.write_coverage(bad, workspace_root=tmp_path, project_slug="demo-furniture", run_id=RUN_ID)
    assert not coverage.coverage_path(tmp_path, "demo-furniture", RUN_ID).exists()
...
def test_write_coverage_rejects_naive_created_at(tmp_path: Path) -> None:
    ...
    with pytest.raises(coverage.CoverageValidationError):
        coverage.write_coverage(record, workspace_root=tmp_path,
                                project_slug="demo-furniture", run_id=RUN_ID)
    assert not coverage.coverage_path(tmp_path, "demo-furniture", RUN_ID).exists()  # nothing written
