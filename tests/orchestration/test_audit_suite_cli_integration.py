...
def _arrange_and_run_transforms(raw_dir: Path, out_dir: str) -> None:
    ...
    lh.write_text(json.dumps({"items": [
        {"url": "https://demo-furniture.example/p1", "performance_score": 0.4, "lcp": 3.2},
    ]}), encoding="utf-8")
    cp.write_text(json.dumps({"items": [
        {"url": "https://demo-furniture.example/p1", "title": "T",
    ...
    sf.write_text(json.dumps({"rows": [
        {"address": "https://demo-furniture.example/p1",
    ...
    gsc.write_text(json.dumps({"rows": [
        {"keys": ["kw", "https://demo-furniture.example/p1"],
         "impressions": 100, "clicks": 5, "position": 3},
        {"keys": ["kw", "https://demo-furniture.example/p2"],
