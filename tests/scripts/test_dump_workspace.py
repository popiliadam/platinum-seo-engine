...
def test_dump_with_explicit_slug_returns_summary_dict(dump_module, tmp_path):
    _make_workspace(tmp_path, slug="vento")

    result = dump_module.dump_workspace(workspace_root=tmp_path, project_slug="vento")

    assert result["project"] == "vento"
...
def test_dump_uses_active_json_when_slug_none(dump_module, tmp_path):
    _make_workspace(tmp_path, slug="vento")
    _write_active_json(tmp_path, "vento")
