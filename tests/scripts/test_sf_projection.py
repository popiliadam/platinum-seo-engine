...
# Real staged SF crawl (live fixtures, 22 CSVs).
_RAW_DIR = Path(
    "/Users/apple/Documents/platinum-seo-workspace/projects/"
    "demo-aluminum-ca/sf-exports/2026-06-02/raw"
)
...
def test_redirect_404_internal_only(projected: dict) -> None:
    ...
    assert hosts == {"demo-aluminum.example"}
...
def test_redirect_404_excludes_external_hosts(projected: dict) -> None:
    ...
    r404 = projected["redirect_404"]
    for r in r404:
        assert "google.com" not in P._host(r["url"]) or P._host(r["url"]) == "demo-aluminum.example"
