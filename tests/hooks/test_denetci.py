...
def _declared(**overrides: object) -> dict:
    marker = {
        ...
        "workflow": "monthly",
        "slug": "demo-furniture",
        "command": "/pseo-run monthly demo-furniture",
    }
...
def test_decide_failed_blocks() -> None:
    ...
    assert action == "block"
    assert reason is not None
    assert "/pseo-run monthly demo-furniture --resume" in reason
...
def test_freshest_returns_fresh_record(tmp_path: Path) -> None:
    ...
    since = 1_000_000.0
    _write_cov(tmp_path, "demo-furniture", "demo-furniture-2026-06-06-ab12",
               {"run_id": "fresh", "verdict": "incomplete"}, since + 100)
    record = freshest_fresh_coverage(tmp_path, "demo-furniture", since)
...
def test_freshest_ignores_stale_record(tmp_path: Path) -> None:
    ...
    since = 1_000_000.0
    _write_cov(tmp_path, "demo-furniture", "demo-furniture-2026-06-01-0001",
               {"run_id": "stale", "verdict": "pass"}, since - 100)
    assert freshest_fresh_coverage(tmp_path, "demo-furniture", since) is None


def test_freshest_returns_greatest_mtime(tmp_path: Path) -> None:
    """9. among two fresh records the GREATEST mtime wins."""
    since = 1_000_000.0
    _write_cov(tmp_path, "demo-furniture", "demo-furniture-2026-06-06-aaaa",
               {"run_id": "older", "verdict": "incomplete"}, since + 10)
    _write_cov(tmp_path, "demo-furniture", "demo-furniture-2026-06-06-bbbb",
...
def _world(
    ...
    *,
    status: str = "declared",
    slug: str = "demo-furniture",
    command: str = "/pseo-run monthly demo-furniture",
    coverage: dict | None = None,
    run_id: str = "demo-furniture-2026-06-06-ab12",
    fresh: bool = True,
) -> Path:
...
def test_main_declared_incomplete_blocks(tmp_path: Path) -> None:
    ...
    home = _world(tmp_path, coverage={
        "run_id": "demo-furniture-2026-06-06-ab12", "verdict": "incomplete",
...
def test_main_declared_pass_is_silent(tmp_path: Path) -> None:
    ...
    home = _world(tmp_path, coverage={
        "run_id": "demo-furniture-2026-06-06-ab12", "verdict": "pass",
...
def test_main_nonstart_blocks_with_command(tmp_path: Path) -> None:
    ...
    data = json.loads(proc.stdout)
    assert data["decision"] == "block"
    assert "/pseo-run monthly demo-furniture" in data["reason"]
...
def test_main_paused_flags_to_stderr(tmp_path: Path) -> None:
    ...
    home = _world(tmp_path, coverage={
        "run_id": "demo-furniture-2026-06-06-ab12", "verdict": "paused",
