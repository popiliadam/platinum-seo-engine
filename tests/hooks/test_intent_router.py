...
def test_route_tier1_bound(tmp_path: Path) -> None:
    ...
    sb.write_session_binding("sess-1", "vento", ws, "2026-06-05T00:00:00")
    result = router.route(
        "aylık bakım yap",
        session_id="sess-1",
        workspace_root=ws,
...
def test_build_marker_declared_validates() -> None:
    m = router.build_marker(
        ...
        slug="vento", command="/pseo-run monthly vento",
    )
    _validator().validate(m)
...
def test_main_tier1_writes_marker_and_prints_one_voice(tmp_path: Path) -> None:
    ...
    home = tmp_path / "home"
    home.mkdir()
    sb.write_session_binding("sid-123", "vento", ws, "2026-06-05T00:00:00")
    ...
    assert "project=vento" in res.stdout
...
def test_main_tier2_supersedes_and_prints_advisory(tmp_path: Path) -> None:
    ...
    home = tmp_path / "home"
    home.mkdir()
    sb.write_session_binding("sid-9", "vento", ws, "2026-06-05T00:00:00")
...
def test_marker_is_rewritten_each_prompt(tmp_path: Path) -> None:
    ...
    home = tmp_path / "home"
    home.mkdir()
    sb.write_session_binding("sid-rw", "vento", ws, "2026-06-05T00:00:00")
