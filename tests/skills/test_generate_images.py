...
def test_plugin_agnostic_mcp_boundary_higgsfield_not_in_mcp_json():
    ...
    server entry into `.mcp.json`. Beklenen 6 server intact (v1.8 Phase 2
    sf added per ADR-039 controlled F-16 break; pre-v1.8 baseline was 3;
    adstark + xquik added for the refresh-audit task-reconciliation phase —
    a second controlled break, completed only once both servers were
    registered in mcp-tool-registry.json and F-24 went back to PASS):
    `gsc`, `dataforseo`, `ScraplingServer`, `sf`, `adstark`, `xquik`."""
    ...
    expected = {"gsc", "dataforseo", "ScraplingServer", "sf", "adstark", "xquik"}
