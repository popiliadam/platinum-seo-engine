...
Discipline:
  - tmp_path fixtures only; the live workspace (`$PSEO_WORKSPACE_ROOT`) is
    read AT-REST in test 5 (live dentnotion config evidence) but never mutated
...
# Live workspace (read-only here; test 5 reads the dentnotion config AT-REST
...
def test_gsc_list_sites_optional_verify() -> None:
    ...
    unset or the dentnotion project is absent."""
    text = SKILL_MD.read_text(encoding="utf-8")
    m = re.match(r"^---\n(.*?)\n---\n", text, re.S)
    fm = yaml.safe_load(m.group(1))
    ...
    if not pilot_cfg.exists():
        pytest.skip(
            f"dentnotion live config absent: {pilot_cfg} "
