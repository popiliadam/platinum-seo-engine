...
hardcode YASAK, e.g. `dentnotion`/`vento`/`eykom`/`bigcattr`/`calitte`/
`lastiksa`/`noraninsaat`/`adstark` slug'lar HTML/CSS içinde geçemez).
...
- No project slug hardcoded inside this skill or its templates
  (`dentnotion`, `vento`, `eykom`, `bigcattr`, `calitte`, `lastiksa`,
  `noraninsaat`, `adstark` are FORBIDDEN tokens — pytest grep verifies).
