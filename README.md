...
**Current:** v1.9.4 — AC-10 sheet projection: `sf_import` now projects raw SF CSVs → master.xlsx's 6 SF sheets (schema-valid + idempotent), with the `tech_seo` issue-category enum solved by routing (no schema change). Live-proven on demo-aluminum (1,427 tests passing, drift-check AMBER).
