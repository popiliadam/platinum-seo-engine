...
> **Skill:** `skills/production/new-blog/SKILL.md` (Phase 11 W1, **wip** — runtime `scripts/production/new_blog.py` henüz yok). NCP satırı → SERP analiz → 5 template render → JSON-LD @graph 5 entity → Foundational Principles 3-layer enforce → 4 artifact yazar (`outputs/blog/{slug}/article.html` + `schema.jsonld` + `meta-tags.json` + `upload-instructions.md`) → onay gate.
>
> **DRAFT-ONLY artifact generator.** Skill `master.xlsx`'e karşı **READ-ONLY**'dir (`new_content_plan` allowed_writers=null, F-1); workbook'a append/update/delete YAPMAZ. Tek state mutasyonu `events.jsonl` audit append'idir.
...
6. Output write — 4 artifact: `outputs/blog/{slug}/article.html` + `schema.jsonld` + `meta-tags.json` + `upload-instructions.md` (markdown article YOK; HTML fragment + ayrı JSON-LD dosyası)
...
- `--mode draft` (default, review öncesi taslak) ve `--mode publish` (immediate) **yalnız** `outputs/blog/{slug}/` artifact'larını yazar; hiçbir mod `master.xlsx`'i değiştirmez (skill READ-ONLY).
- new_content_plan lifecycle geçişi (PLANNED/DONE) new-blog'un işi DEĞİL — ayrı `mark-done` skill'i sahiplenir (artifact üretimi ile lifecycle yazımı ayrık sorumluluklar).
...
- Skill: `skills/production/new-blog/SKILL.md` (Phase 11 W1, **wip** — runtime Phase 11 Wave 2'ye ertelendi; SKILL.md spec-lock)
- Scripts: `scripts/state/events_writer.py` (`append_work`) + `scripts/state/workflow_runner.py` + `scripts/reporting/render_template.py` + `scripts/budget/check_budget.py` (workbook yazımı YOK — read-only skill)
...
- MCP (**skill-level** — new-blog skill çağırır, bu komut DEĞİL; skill frontmatter `mcp_tools`): `mcp__higgsfield__generate_image` (required) + `mcp__higgsfield__job_status` (optional polling) + `mcp__gsc__search_analytics` (SERP delta) + `mcp__dataforseo__serp_organic_live_advanced` (SERP analiz)
